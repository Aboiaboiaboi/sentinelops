...
variable "root_volume_size_gb" {
  description = <<-EOT
    EBS root volume size. 30 GB: the OS, the app and observability Docker
    images, the five sandboxed tool images (Gitleaks, Trivy, Semgrep,
    Hadolint, and Checkov — Checkov the largest of the five at ~170 MB,
    measured), the Trivy/Semgrep cache (~1.1 GB), and 14 days of retained
    Prometheus + Loki data (~1.5 GB combined) — with headroom.
  EOT
  type        = number
  default     = 30
}
