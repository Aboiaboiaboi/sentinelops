    ...
    <div className="mx-auto max-w-3xl">
      <div className="flex flex-col gap-3">
        {CHECK_CATALOG.map((group) => (
          <details
            key={group.category}
            className="group rounded-lg border border-border/60 bg-card/40 open:bg-card/60"
          >
            <summary className="flex cursor-pointer list-none items-center gap-3 p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background [&::-webkit-details-marker]:hidden">
              <ChevronDown className="size-4 shrink-0 text-muted-foreground transition-transform group-open:rotate-180" />
              <span className="font-display text-base font-semibold">{group.category}</span>
              <span className="font-mono text-sm text-primary-bright">{group.weight}</span>
              <span className="ml-auto text-sm text-muted-foreground">
                {group.checks.length} checks
              </span>
            </summary>
            <ul className="flex flex-col gap-2 px-4 pb-4 pl-11">
              {group.checks.map((check) => (
                <li key={check.id} className="flex items-center gap-2 text-sm text-foreground">
                  <span>{check.title}</span>
                  {check.tool && (
                    <Badge variant="outline" className="text-primary-bright">
                      {check.tool}
                    </Badge>
                  )}
                </li>
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">
        Every one of the {TOTAL_CHECK_COUNT} checks reports passed, failed,
