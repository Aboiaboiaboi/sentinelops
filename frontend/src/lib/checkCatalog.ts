/**
 * The 31 checks, for the public /how-it-works page.
 *
 ...
 * module, applied on failure rather than declared on the check. Three
 * checks (the tool-backed ones) scale at runtime with severity/volume, and
 * Deployment's eight impacts sum to 27, not its 15 weight, because it has
 * two mutually exclusive scoring paths depending on what the repo has.
...
export type CheckTool = 'Gitleaks' | 'Trivy' | 'Semgrep';
